...
class UniformSampler(DataSampler):
    def __init__(self, n_dims, min_val=1, max_val=2):
        """
            Integer sampler that returns positive integers in [min_val, max_val)
        """
        super().__init__(n_dims)
    ...
    def sample_xs(self, n_points, b_size, n_dims_truncated=None, seeds=None):
        if seeds is None:
            xs_b = torch.empty(size=(b_size, n_points, self.n_dims), device="cuda").uniform_(*(self.min_val, self.max_val))
        else:
            xs_b = torch.zeros(size=(b_size, n_points, self.n_dims), device="cuda")
            generator = torch.Generator()
            assert len(seeds) == b_size
            for i, seed in enumerate(seeds):
                generator.manual_seed(seed)
                xs_b[i] = torch.empty(size=(n_points, self.n_dims), generator=generator, device="cuda").uniform_(*(self.min_val, self.max_val))
        if n_dims_truncated is not None:
            xs_b[:, :, n_dims_truncated:] = 0
        return xs_b
